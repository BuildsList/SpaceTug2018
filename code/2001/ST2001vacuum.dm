...
proc/MaintainVacuum()
	...
	to_chat(world,"<font color='red'><small>Подключаем атмос...</font>")

	var/K

	to_chat(world,"<font color='red'><small>Разгерметизации готовы!</font>")

	var/mob/M
	var/oldPress

	to_chat(world,"<font color='red'><small>Давление готово!</font>")
		...
		if(!flux)
			if(currentAir < lastStable)
				...
				if(airReduction >= originalAir * 0.01)
					...
					SayPA("Бортовой Компьютер", "ВНИМАНИЕ. Запас кислорода - \
